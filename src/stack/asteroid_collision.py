...
of the asteroid in the array represent their relative position in space.
...
Output: [-6,2,4]
Explanation:
The asteroid -6 makes the asteroid 3 and 5 explode, and then continues going left. On
the other side, the asteroid 2 destroys -1. Since 2 and 4 are both moving right, they
never collide.
...
class Solution:
    def asteroidCollision(self, asteroids):
        ...
        Args:
            asteroids: List[int] - asteroids; sign gives direction, absolute value gives size

        Returns:
            List[int] - remaining asteroids
