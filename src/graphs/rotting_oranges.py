...
Output: -1
Explanation:
The orange in the bottom left corner (row 2, column 0) is never rotten, because rotting
...
class Solution:
    def orangesRotting(self, grid):
        """
        Minutes until no fresh orange remains.

        Args:
            grid: List[List[int]] - 0 empty, 1 fresh orange, 2 rotten orange

        Returns:
            int - number of minutes, or -1 if impossible
