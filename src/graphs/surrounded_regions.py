...
- Surround: A region is surrounded if none of the 'O' cells in that region are on the
  edge of the board. Such regions are completely enclosed by 'X' cells.
...
class Solution:
    def solve(self, board):
        """
        Capture every region of 'O' that is surrounded by 'X'.

        Args:
            board: List[List[str]] - grid of 'X' and 'O'
...
if __name__ == "__main__":
    ...
    print(f"Test 1: {board}")

    # Test case 2
    board = [["X"]]
    solution.solve(board)
    print(f"Test 2: {board}")
