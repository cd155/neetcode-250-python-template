...
detectSquares.count([14, 8]); // return 0. The query point cannot form a square with any
...
class DetectSquares:
    def __init__(self):
        """
        Initialize the object with an empty data structure.
    ...
    def count(self, point):
        """
        Count the ways to form axis-aligned squares with point as one corner.

        Args:
            point: List[int] - [x, y] point

        Returns:
            int - number of positive-area squares
...
if __name__ == "__main__":
    ...
    detectSquares.add([11, 2])
    detectSquares.add([3, 2])
    print("count([11, 10]):", detectSquares.count([11, 10]))  # 1
    print("count([14, 8]):", detectSquares.count([14, 8]))  # 0
    detectSquares.add([11, 2])
    print("count([11, 10]):", detectSquares.count([11, 10]))  # 2
