...
class Solution:
    def findItinerary(self, tickets):
        """
        Reconstruct the itinerary that starts at JFK and uses every ticket once.

        Args:
            tickets: List[List[str]] - tickets[i] = [from, to] airline tickets

        Returns:
            List[str] - itinerary with the smallest lexical order
