...
Return any matrix that satisfies the conditions. If no answer exists, return an empty
matrix.
...
Output: []
Explanation:
From the first two conditions, 3 has to be below 1 but the third conditions needs 3 to
...
class Solution:
    def buildMatrix(self, k, rowConditions, colConditions):
        """
        Build a k x k matrix that satisfies the row and column conditions.

        Args:
            k: int - numbers 1 to k must be placed
            rowConditions: List[List[int]] - rowConditions[i] = [above, below]
            colConditions: List[List[int]] - colConditions[i] = [left, right]

        Returns:
            List[List[int]] - any valid matrix, or an empty matrix
...
if __name__ == "__main__":
    ...
    # Test case 2
    result = solution.buildMatrix(3, [[1, 2], [2, 3], [3, 1], [2, 3]], [[2, 1]])
