...
- void put(int key, int value) Update the value of the key if present, or inserts the
  ...
  and remove the least frequently used key before inserting a new item. For this
  problem, when there is a tie (i.e., two or more keys with the same frequency), the
...
class LFUCache:
    ...
    def get(self, key):
        """
        Get the value of key and increase its use count.

        Args:
            key: int - key

        Returns:
            int - value, or -1 if key is absent
    ...
    def put(self, key, value):
        """
        Insert or update key, evicting the least frequently used key when full.
...
if __name__ == "__main__":
    ...
    lfuCache.put(1, 1)
    lfuCache.put(2, 2)
    print("get(1):", lfuCache.get(1))  # 1
    lfuCache.put(3, 3)
    print("get(2):", lfuCache.get(2))  # -1
    print("get(3):", lfuCache.get(3))  # 3
    lfuCache.put(4, 4)
    print("get(1):", lfuCache.get(1))  # -1
    print("get(3):", lfuCache.get(3))  # 3
    print("get(4):", lfuCache.get(4))  # 4
