...
number n. Each CPU interval can be idle or allow the completion of one task. Tasks can
be completed in any order, but there's a constraint: there has to be a gap of at least n
...
applies to task B. In the 3rd interval, neither A nor B can be done, so you idle. By the
...
class Solution:
    def leastInterval(self, tasks, n):
        """
        Least number of CPU intervals to finish all tasks with cooldown n.

        Args:
            tasks: List[str] - task labels (uppercase letters)
            n: int - cooldown between two identical tasks

        Returns:
            int - minimum number of intervals
...
if __name__ == "__main__":
    ...
    # Test case 2
    result = solution.leastInterval(["A", "C", "A", "B", "D", "B"], 1)
