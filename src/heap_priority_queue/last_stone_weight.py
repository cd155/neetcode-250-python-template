...
and smash them together. Suppose the heaviest two stones have weights x and y with x <=
...
we combine 1 and 1 to get 0 so the array converts to [1] then that's the value of the
last stone.
...
class Solution:
    def lastStoneWeight(self, stones):
        """
        Smash the two heaviest stones together until at most one remains.

        Args:
            stones: List[int] - stones[i] is the weight of the i-th stone

        Returns:
            int - weight of the last stone, or 0
...
if __name__ == "__main__":
    ...
    # Test case 2
    result = solution.lastStoneWeight([1])
