...
arranged in a row, and each stone has an associated value which is an integer given in
...
Return "Alice" if Alice will win, "Bob" if Bob will win, or "Tie" if they will end the
...
Output: "Bob"
Explanation:
Alice will always lose. Her best move will be to take three piles and the score become
...
Output: "Alice"
Explanation:
Alice must choose all the three piles at the first move to win and leave Bob with
negative score.
...
Remember that both play optimally so here Alice will choose the scenario that makes her
win.
...
Output: "Tie"
Explanation:
Alice cannot win this game. She can end the game in a draw if she decided to choose all
...
class Solution:
    def stoneGameIII(self, stoneValue):
        """
        Decide the winner of Stone Game III when both play optimally.

        Args:
            stoneValue: List[int] - stoneValue[i] is the value of the i-th stone
...
if __name__ == "__main__":
    ...
    # Test case 1
    result = solution.stoneGameIII([1, 2, 3, 7])
    print(f"Test 1: {result!r}")

    # Test case 2
    result = solution.stoneGameIII([1, 2, 3, -9])
